...
**Latest delivery (2026-09-17)**: Added `tests/circulation-contracts.test.ts` and exported the typed `buildRequestHash` contract. The suite now verifies deterministic idempotency hashes, changed-request differentiation, tenant context presence and server-side circulation permission denial. Added `tests/postgres-integrity.test.ts` and `test:integration` scripts. The connected database passed migration reachability, active-loan/hold index checks, and reproducible Tenant A/B fixture isolation (3 passed, 0 skipped). `pnpm db:seed` now loads the independent fixture safely through upserts. The dedicated PostgreSQL concurrency suite now passes 2/2: competing active loan rejection and transaction rollback. The Circulation Desk Loans tab is connected to the scoped read service and renders live PostgreSQL records/metrics. The transaction drawer now invokes the server-side `returnItem` mutation with authorization context and a generated idempotency key, then refreshes the live queue; the control includes English/Spanish labels and pending/error feedback. Checkout, renew, hold and secondary queues remain pending. The development seed now provisions default credential users for PLATFORM_ADMIN, TENANT_ADMIN, LIBRARIAN and NORMAL_USER roles with idempotent role assignments and hashed passwords. Circulation Desk UI was refined with semantic tab navigation, accessible transaction dialog semantics, responsive queue layout, a deliberate empty state, stronger focus/hover feedback, and a more polished sticky application header; browser verification passed at desktop 1280×800 and mobile 384×587. The public landing page and application shell were redesigned into a premium editorial system using ink navy, warm ivory, brass, and sage tokens; the landing experience was verified at desktop 1280×800 and mobile 384×587.
...
| P0.5 | Circulation | PARTIAL | Loans tab reads live scoped PostgreSQL data; return mutation is now connected through a server action with idempotency and bilingual UX; checkout/renew/hold UI and secondary queues remain |
