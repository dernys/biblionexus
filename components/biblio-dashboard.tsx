...
function Sidebar({ collapsed, mobileOpen, onCollapse, onClose, locale }: { collapsed: boolean; mobileOpen: boolean; onCollapse: () => void; onClose: () => void; locale: Locale }) {
  ...
  return <aside className={cn('shrink-0 border-r border-border bg-sidebar transition-[width] duration-200', mobileOpen ? 'fixed inset-y-0 left-0 z-40 flex w-64 flex-col shadow-xl' : 'hidden lg:flex lg:flex-col', collapsed ? 'lg:w-20' : 'lg:w-64')}>
    ...
    <nav className="flex-1 p-3" aria-label="Primary navigation"><div className="flex flex-col gap-1">{navigation.map(({ label, href, icon: Icon }) => { const active = pathname === href; const translatedLabel = copy[label.toLowerCase() as keyof typeof copy] ?? label; return <button key={href} type="button" title={collapsed ? label : undefined} onClick={() => { router.push(href); onClose() }} className={cn('flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground', active && 'bg-sidebar-accent font-medium text-sidebar-accent-foreground')}><Icon data-icon="inline-start" />{!collapsed && <span>{translatedLabel}</span>}</button> })}</div></nav>
