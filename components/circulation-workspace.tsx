...
export function CirculationWorkspace({ initialTab = 'loans', overview }: { initialTab?: TabKey; overview?: { records: string[][]; transactions: { loanId: string; memberId: string; itemId: string; branchId: string; barcode: string }[]; metrics: { label: string; value: string; note: string }[] } }) {
  ...
  const copy = locale === 'es' ? { language: 'Idioma', english: 'English', spanish: 'Español', processing: 'Procesando…', confirmReturn: 'Confirmar devolución', selectLoan: 'Selecciona un préstamo activo para procesar una devolución', returnCompleted: 'Devolución completada', unable: 'No se pudo completar la operación' } : { language: 'Language', english: 'English', spanish: 'Español', processing: 'Processing…', confirmReturn: 'Confirm return', selectLoan: 'Select an active loan to process a return', returnCompleted: 'Return completed', unable: 'Unable to complete transaction' }
  const liveRecords = tab === 'loans' && overview ? overview.records : records[tab]
  const selectedIndex = selected ? liveRecords.findIndex((row) => row[2] === selected) : -1
  const selectedTransaction = overview?.transactions[selectedIndex]
  ...
  return <div className="space-y-7 pb-10">
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_300px]">
      <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">{config.label} queue</p><p className="mt-1 text-xs text-muted-foreground">{config.description}</p></div><div className="flex gap-2"><label className="flex min-w-0 items-center gap-2 rounded-lg border border-border bg-background px-3"><Search className="size-4 text-muted-foreground" /><span className="sr-only">Search {config.label}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter queue" className="w-full bg-transparent py-2 text-sm outline-none" /></label><button className="rounded-lg border border-border p-2 text-muted-foreground hover:bg-muted" aria-label="Filter"><Filter className="size-4" /></button></div></div><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><caption className="sr-only">{config.label} circulation queue</caption><thead className="bg-muted/30 text-xs text-muted-foreground"><tr>{['Record', tab === 'transfers' ? 'Route' : tab === 'holds' ? 'Requestor' : 'Member', 'Reference', 'Status', 'Next step'].map((head) => <th className="px-4 py-3 font-medium" key={head}>{head}</th>)}<th className="px-4 py-3" /></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={6} className="px-6 py-14 text-center"><div className="mx-auto grid size-11 place-items-center rounded-2xl bg-muted text-muted-foreground"><Search className="size-5" /></div><p className="mt-4 text-sm font-medium">No matching records</p><p className="mt-1 text-xs text-muted-foreground">Try a different title, member, or barcode.</p></td></tr> : rows.map((row) => <tr key={row[2]} className="border-t border-border hover:bg-muted/20"><td className="px-4 py-4"><p className="font-medium">{row[0]}</p><p className="mt-1 text-xs text-muted-foreground">{tab === 'transfers' ? 'Collection movement' : 'Central Library'}</p></td><td className="px-4 py-4 text-muted-foreground">{row[1]}</td><td className="px-4 py-4 font-mono text-xs text-muted-foreground">{row[2]}</td><td className="px-4 py-4"><Status value={row[3]} /></td><td className="px-4 py-4 text-muted-foreground">{row[4]}</td><td className="px-4 py-4 text-right"><button onClick={() => setSelected(row[2])} className="rounded-lg p-2 text-muted-foreground hover:bg-muted" aria-label={`Open ${row[2]}`}><ChevronDown className="size-4 -rotate-90" /></button></td></tr>)}{rows.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-muted-foreground">No transactions match this filter.</td></tr>}</tbody></table></div></section>
