...
export async function checkout(params: CheckoutParams) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const member = await tx.member.findFirst({ where: { id: memberId, branch: { id: { in: Array.from(context.branchIds) } } } })
    ...
    const loan = await tx.loan.create({ data: { memberId, itemId, branchId, dueDate: addDays(new Date(), daysToLoan) }, include: { member: true, item: true } })
...
export async function returnItem(params: ReturnParams) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const now = new Date(); const overdueDays = Math.max(0, Math.floor((now.getTime() - loan.dueDate.getTime()) / 86_400_000)); const fineAmount = new Prisma.Decimal(overdueDays * 0.5)
...
export async function renewLoan(params: RenewParams) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    return { success: true, loanId, newDueDate: newDueDate.toISOString(), renewalsRemaining: 3 - updated.renewals }
  })
}
